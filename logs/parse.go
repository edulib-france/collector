...
var LogPrefixScalingoRegexp = regexp.MustCompile(`(?s)^` + `time=` + TimeRegexp + `, pid=` + PidRegexp + `  db=` + DbRegexp + `, usr=` + UserRegexp + `, client=` + HostRegexp + ` , app=` + AppBeforeCommaRegexp + `, line=` + LogLineCounterRegexp + ` ` + LevelAndContentRegexp)
